...
void main() {
    ...
    if (uSharpness > 0.01 || uDefinition > 0.01) {
        ...
        vec3 laplacian = (n + s + e + w) * 0.25;
        vec3 detail = rgb - laplacian;

        if (uSharpness > 0.01) {
            rgb += detail * (uSharpness * 1.5);
        }
        if (uDefinition > 0.01) {
            rgb += sign(detail) * pow(abs(detail), vec3(0.8)) * (uDefinition * 0.8);
        }
    }
    ...
    if (abs(uBrightness) > 0.001) {
        rgb += vec3(uBrightness * 0.25);
    }

    // 5. Contrast & Black Point
    if (abs(uContrast - 1.0) > 0.001 || uBlackPoint > 0.001) {
        rgb = (rgb - 0.5) * uContrast + 0.5;
        if (uBlackPoint > 0.001) {
            rgb = max(vec3(0.0), rgb - vec3(uBlackPoint * 0.15)) / max(0.0001, 1.0 - uBlackPoint * 0.15);
        }
    }

    // 6. Luminance for tone mapping
